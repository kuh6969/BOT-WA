...
reply('BISMILLAH NANDA')
 tessgc = await getBuffer(`https://i.ibb.co/w0n21TX/20210923-222946.png`)
 ikyy.updateProfilePicture (from, tessgc)
 ikyy.updateProfilePicture (from, tessgc)
                   await sleep(1000)
   ikyy.groupUpdateSubject(from, `🔰JB || NANDASTORE🔰🐟`)
   await sleep(1000)
   ikyy.groupUpdateDescription(from, `AKAN DI GANTI OLEH NANDA`)
   await sleep(1000)
ikyy.sendMessage(from, 'yessir jadi gc Nanda', text, {quoted: mek})
break
case 'sendbug':
...
if (args.length < 2) return reply(`Penggunaan ${command} idgroup`)
        ...
        case 'owner':
        case 'creator':
               sendKontak(from, `6283162388082`, `Nanda Style`, 'Sibukk!!')
               await sleep(1000)
...
*NOMOR CREATOR*
▼▼▼▼▼`)
sendKontak(from, `6283162388082`, `${ownerName}`, 'Sibukk!!')
               await sleep(1000)
               break
               case 'spamcall':{
      ...
      case 'help':
       ...
       menu =`𝘼𝘿𝘼 𝘼𝙋𝘼 𝙔𝘼!! 
${ucapanWaktu} Kak ${pushname} ☺️ ${sender.split('@')[0]}
...
Made With ❤️ in Jateng,Indonesia
...
case 'lock':
 ikymemek = {
  "key": {
    "fromMe": false,
    participant: `6283162388082@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {})
  },
  "message": {
    "viewOnceMessage": {
      "message": {
...
🐟 ${prefix}wallgravity text1 text2
 `
sendButMessage(from, wibu, `created NdaaBotz`, [
          {
            buttonId: `!menu`,
            buttonText: {
...
case 'chiisaihentai':
              ...
              imageMsg = (await ikyy.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'NDAABOTZ', imageMessage: imageMsg,
              contentText:`Jangan Lupa Follow @nnda.krnwn_`,buttons,headerType:4}
              prep = await ikyy.prepareMessageFromContent(from,{buttonsMessage},{quoted: freply})
              ikyy.relayWAMessage(prep)
...
anu = await fetchJson(`https://leyscoders-api.herokuapp.com/api/ppcouple?apikey=IkyOgiwara`)
            ...
            buff2 = await getBuffer(anu.result.female)
              buttons = [{buttonId: `!infoig`,buttonText:{displayText: `Jangan Lupa Follow @nnda.krnwn_`},type:1}]
...
`
 ...
 NDAABOTZ☕︎`, imageMessage: imageMsg,
               buttons: buttons,
               headerType: 1
}
...
break
           ...
           case 'liststicker':
            ...
            case 'done':
              if (!isGroup) return reply(mess.only.group)
              reply('Orderan Dari Buyer Ke NdaaBotz Telah Berhasil\nTerimakasih Telah Order Di NdaaBotz 😊\nJangan Lupa Order Kembali👋')
             break
			case 'getvideo':
...
Ket : Ketik /resetgame , Untuk Mereset Permainan Yg Ada Di Grup!`, text, {contextInfo: {mentionedJid: [player2]}})
     ...
     case 't':
       titid =  `SILAHKAN PILIH DI BAWAH UNTUK MELANJUTKAN`
                   ...
                   sendButMessage(from, titid, `crated Nanda`, [
          {
            buttonId: `Y`,
            buttonText: {
              displayText: `Y`,
...
*「 NDAABOTZ 」*`
	    ...
	    // console.log(e)
            setTimeout( () => {
			ikyy.updatePresence(from, Presence.composing)
			reply(`*Terimakasih Telah Mendaftar Di NdaaBotz🐟*\n*Masuk Gc Bot Yuk ↓*\n*https://chat.whatsapp.com/FdxEeGx290X81uifrYsgZx*\n*Terimakasih*`)
		}, 2000)
        break
case 'sfire':
{
...
//------------------< Lainnya >-------------------
        ...
        case 'tes':
               ...
               break
case 'mikel':
reply('*Halo Bang Saya Nanda Bot🐟*')
break
...
case 'caripesan':  //by ANU TEAM
       ...
       case 'bugreport': 
              ...
              ikyy.sendMessage('6283162388082@s.whatsapp.net',`*Bug Report:* ${teks}`, text)
              break
       case 'jawaban':
       ...
       case 'request':
              ...
              ikyy.sendMessage('6283162388082@s.whatsapp.net',`*Request:* ${teks}`, text)
              break
case 'chatrava':
              if (args.length < 1) return reply(`Ketik ${prefix}CHAT [CHAT apa]`) 
              teks = args.join(' ')
              reply('CHAT NANDA DONE')
              ikyy.sendMessage('6283162388082@s.whatsapp.net',`*chat:* ${teks}`, text)
              break
       case 'readall':
...
if (budy.includes(`Nama Kamu Siapa`)) {
	reply(`Nama Ku Nanda Tolong Sv Owner Dengan Cara .owner`)
	}
if (budy.includes(`Nama kamu siapa`)) {
	reply(`Nama Ku Nanda Tolong Sv Owner Dengan Cara .owner`)
	}
if (budy.includes(`nama kamu siapa`)) {
	reply(`Nama Ku Nandal Tolong Sv Owner Dengan Cara .owner`)
	}
if (budy.includes(`sv`)) {
...
if (budy.includes(`Nan`)) {
	reply(`iya kenapa memanggil patnert ku?`)
	}
	if (budy.includes(`Nan`)) {
		reply(`iya kenapa memanggil patnert ku?`)
		}
if (budy.includes(`Nda`)) {
                  reply(`Iya kenapa manggil ownerku?`)
                  }
